    ...
    <section>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          {urgentCount > 0 && (
            <div className="flex items-center gap-1 text-[11px] font-semibold text-rose-600 dark:text-rose-300 bg-rose-500/15 border border-rose-500/30 px-2.5 py-0.5 rounded-full animate-pulse">
        ...
        <Link
          href="/courses"
          className="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground bg-muted/60 hover:bg-muted px-3 py-1.5 rounded-full transition-all border border-border"
        >
      ...
      {/* Empty State vs List */}
      {upcoming.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 px-4 rounded-2xl border border-border/80 dark:border-white/[0.08] bg-card/60 dark:bg-white/[0.03] backdrop-blur-xl text-center transition-colors">
          <div className="h-12 w-12 rounded-2xl bg-gradient-to-tr from-[#00D2FF]/20 to-[#2F7BFF]/20 border border-[#2F7BFF]/30 flex items-center justify-center mb-3 text-[#2F7BFF] dark:text-[#38BDF8] shadow-[0_0_20px_rgba(47,123,255,0.2)]">
            <CheckCircle2 className="h-6 w-6" strokeWidth={2} />
          </div>
          <p className="text-sm font-bold text-foreground">{t.dashboard.noDeadlines}</p>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  status === "urgent"
                    ? "border-rose-500/30 bg-rose-50/50 dark:bg-gradient-to-r dark:from-rose-500/[0.08] dark:to-white/[0.02] hover:border-rose-500/50 shadow-sm dark:shadow-[0_0_20px_rgba(244,63,94,0.12)]"
                    : "border-border/80 dark:border-white/[0.08] bg-card/90 dark:bg-gradient-to-b dark:from-white/[0.05] dark:to-white/[0.01] hover:border-[#2F7BFF]/40",
                  "active:scale-[0.99]"
                )}
              >
                ...
                <div className="flex-1 min-w-0 z-10">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-foreground truncate transition-colors group-hover:text-[#2F7BFF] dark:group-hover:text-[#38BDF8]">
                      {assignmentTitle}
                    </p>
                  </div>
                ...
                <div className={cn("flex-shrink-0 flex flex-col gap-1.5 z-10", isRtl ? "text-left items-end" : "text-right items-end")}>
                  {status === "urgent" ? (
                    <div className="flex items-center gap-1.5 text-[11px] font-semibold text-rose-600 dark:text-rose-300 bg-rose-100 dark:bg-rose-500/20 border border-rose-200 dark:border-rose-500/30 px-2.5 py-1 rounded-lg shadow-sm">
                      <Clock className="h-3 w-3" />
                      <span>{t.dashboard.urgentBadge}: {relTime}</span>
                    </div>
                  ) : status === "soon" ? (
                    <div className="flex items-center gap-1.5 text-[11px] font-semibold text-amber-700 dark:text-amber-300 bg-amber-100 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 px-2.5 py-1 rounded-lg">
                      <Clock className="h-3 w-3" />
                      <span>{t.dashboard.soonBadge}: {relTime}</span>
                    </div>
                  ) : (
                    <span className="flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground bg-muted/70 dark:bg-white/[0.06] border border-border/80 dark:border-white/[0.08] px-2.5 py-1 rounded-lg">
